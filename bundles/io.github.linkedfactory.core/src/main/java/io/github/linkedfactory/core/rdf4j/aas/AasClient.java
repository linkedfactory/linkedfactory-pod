...
public class AasClient implements Closeable {
	...
	private IRI nodeToIRI(JsonNode node, URI rootUri, String shortIdPath, String genIdPath) {
		...
		if (id != null) {
			...
			subject = valueFactory.createIRI(rootUri.appendFragment(genIdPath).toString());
		}
		...
		while (records.hasNext()) {
			...
			if (nodeValue.isArray() && (USE_AAS_INDEX_PROPERTY || !orderedElements.contains(recordNode.getKey()))) {
				...
				for (JsonNode element : nodeValue) {
					...
					cache.add(subject, predicate, object);
					if (addIndex) {
						IRI indexPredicate = valueFactory.createIRI(AAS_INDEX_PROPERTY.toString());
						cache.add(subject, indexPredicate, valueFactory.createLiteral(i));
					}
					i++;
				}
			} else {
	...
	private Value nodeToRdfValue(JsonNode node, URI rootUri, String shortIdPath, String genIdPath) {
		if (node == null) {
