...
public final class Varint {
	 ...
	 * SQLite</a>.
	 *
	 * @param b     byte array for writing the bytes
	 * @param pos   target position within the byte array
	 * @param value value to encode
	 * @see #writeUnsigned(ByteBuffer, long)
	 */
	public static void writeUnsigned(byte[] b, int pos, final long value) {
		if (value <= 240) {
			b[pos] = (byte) value;
		} else if (value <= 2287) {
			b[pos] = (byte) ((value - 240) / 256 + 241);
			b[pos + 1] = (byte) ((value - 240) % 256);
		} else if (value <= 67823) {
			b[pos] = (byte) 249;
			b[pos + 1] = (byte) ((value - 2288) / 256);
			b[pos + 2] = (byte) ((value - 2288) % 256);
		} else {
			int bytes = descriptor(value) + 1;
			b[pos] = (byte) (250 + (bytes - 3));
			writeSignificantBits(b, pos, value, bytes);
		}
	}
	 ...
	 * @param bytes number of significant bytes
	 */
	private static void writeSignificantBits(byte[] b, int pos, long value, int bytes) {
		int i = pos;
		while (bytes-- > 0) {
